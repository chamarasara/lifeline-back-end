const PackingMaterialInventory = require('../../models/inventory/PackingMaterialInventory');
const Count = require('../../models/counter/count');
const mongoose = require('mongoose');
const moment = require('moment');
const fs = require("fs");
const PDFDocument = require("pdfkit");
const PackingMaterialMaster = require('../../models/master/PackingMaterialMaster');
//Add new purchase order
exports.add_new_packingmaterial_inventory = (req, res, next) => {
    console.log(req)
    Count.findOneAndUpdate({ id: 'grnNumberPm' }, { $inc: { seq: 1 } }, { "new": true }, (error, doc) => {
        console.log("reqqqqqq", req.body.packingMaterials)
        if (doc) {
            ...
            function grnNumber() {
                ...
                return "GRNP" + year.toString() + month.toString() + doc.seq
            }
            ...
            const packingMaterialInventory = new PackingMaterialInventory({
            ...
            packingMaterialInventory.save()
                .then(result => {
                    console.log("packing material inventory save", result.packingMaterials);
                })
                .catch(err => console.log(err));
            res.status(200).json({
                //message: 'New Raw Material successfully created.',
                packingMaterialInventory: packingMaterialInventory
            });
            //     }
            // )
        }
...
//Get all invoices
exports.all_packingmaterial_inventory = (req, res, next) => {
    PackingMaterialInventory.aggregate(
        [
            {
                '$lookup': {
...
//Get single GRN
exports.single_packingmaterial_inventory = (req, res, next) => {
    PackingMaterialInventory.aggregate(
        [
            {
                '$match': {
...
//Print GRN
exports.print_grn_packingmaterial_inventory = (req, res, next) => {
...
//Search invoices
exports.search_packingmaterial_inventory = (req, res, next) => {
