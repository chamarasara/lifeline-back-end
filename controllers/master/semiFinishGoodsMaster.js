const SemiFinishGoodMaster = require('../../models/master/SemiFInishGoodsMaster');
const mongoose = require('mongoose');
//Add new packing material 
exports.semi_finish_good_add_new = (req, res, next) => {
    console.log(req.body)
    const semiFinishGoodMaster = new SemiFinishGoodMaster({
    ...
    res.status(200).json({
        ...
        semiFinishGoodMaster: semiFinishGoodMaster
    });
}
//Get all raw materials
exports.semi_finish_good_get_all = (req, res, next) => {
    SemiFinishGoodMaster.find()
        .exec()
        .then(docs => {
            console.log(docs);
...
exports.semi_finish_good_get_one = (req, res, next) => {
    id = req.params._id;
    SemiFinishGoodMaster.findById(id)
        .exec()
        .then(doc => {
            if (doc) {
...
//Update raw material
exports.update_semi_finish_good = (req, res, next) => {
    ...
    // }
    SemiFinishGoodMaster.update({ _id: id }, { $set: req.body })
        .exec()
        .then(result => {
            SemiFinishGoodMaster.findById(id)
                .then(docs => {
...
//Delete raw material
exports.delete_semi_finish_good = (req, res, next) => {
    const id = req.params._id;
    SemiFinishGoodMaster.remove({ _id: id })
        .exec()
        .then(result => {
